...
static char* guise_string(int guise)
{
	...
	static char ffcxmm[] = "psm_transmit";
...
static char* filetype_guise_string(int guise, bool longGuise)
{
	...
	static char ffcxmm[] = ".M";
	static char ffcxmmlng[] = ".XMM";
...
{
    char shortext[max_namelen+1] = {0};
    char longext[max_namelen+1] = {0};
    ...
    strncpy(shortext, shortguise, max_namelen);
    strncat(shortext, shortformat, max_namelen - strlen(shortguise));

    strncpy(longext, longguise, max_namelen);
    strncat(longext, longformat, max_namelen - strlen(longguise));
		
	if(filename_length>4)
	{
		char *filename_short = (char*) malloc(5);
		if (NULL == filename_short) return filename_has_extension;

		strncpy(filename_short, filename + (filename_length - 4), 4);
        filename_short[4] = '\0';
		...
		else if(filename_length>8)
		{
			...
			if (NULL == filename_long) return filename_has_extension;
			strncpy(filename_long, filename + (filename_length - 8), 8);
            filename_long[8] = '\0';
    ...
    if(header || file_ptr->format == FFTEXT)
    {
        ...
        else
		{
			...
			if (input_output_buflen < required)
			{
				...
				input_output_buffer = (char *)malloc(required * sizeof(char));
			...
			memcpy_s(input_output_buffer, input_output_buflen, buffer, buffer_len);
	...
	const int* namlen, const int* skiphd, int* strid, int* ifail)
{
    ...
    *strid = null_strid;
    ...
    strncpy(keyname, name, max_namelen);
    keyname[max_namelen] = '\0';

    strncpy(filename_short, name, max_namelen);
    filename_short[max_namelen] = '\0';

    strncpy(filename_long, name, max_namelen);
    filename_long[max_namelen] = '\0';
    ...
    /* We now want to open the file for reading. Open binary or text file as appropriate. */
	/* The return value of fopen_s is not recorded and checked, as later code checks for a valid filestream. */
    ...
    if(*format == FFBNRY)
        ...
        stream = fopen(filename_short, g_fopen_mode_read_binary);
    else
        stream = fopen(filename_short, g_fopen_mode_read_text);

	/* If there was a failure in opening the file, the cause could be that the file has a long file extension. */
	/* So try the filename based on the key with a long file extension.  */
    if(stream == 0)
    {
		...
		{
			/* The return value of fopen_s is not recorded and checked, as later code checks for a valid filestream. */
			stream = fopen(filename_long, g_fopen_mode_read_binary);
		}
		else
		{
            /* The return value of fopen_s is not recorded and checked, as later code checks for a valid filestream. */
			stream = fopen(filename_long, g_fopen_mode_read_text);
		}
		if(stream == 0)
		{
			*ifail = FR_not_found;
	...
	// Store offset - the offset which Parasolid passes to 
    // the function registered as FFSKXT is relative to
    // the start of the Parasolid XT data in the file.	
    file_ptr->xt_offset = ftello(file_ptr->stream);
	...
	int* strid, int* ifail)
{
	...
	int extn_padding_len	= 10; /* enough space for known file extensions */
    ...
    *strid = null_strid;
	...
	/* Check here to see if input key name is too long, allowing additional space for file extensions */

	if(*namlen > (max_namelen - extn_padding_len) )
    {
        *ifail = FR_bad_name;
        return;
    }
    ...
    strncpy(filename, name, max_namelen);
    filename[max_namelen] = '\0';
